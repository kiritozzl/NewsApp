...
public class DbFavNews {
    ...
    private static final String TAG = "DbFavNews";
    ...
    public void saveFavorite(News news){
        if (news != null){
            ...
            values.put(DbHelper.COLUMN_NEWS_IMG,news.getImgUrl());
            values.put(DbHelper.COLUMN_NEWS_TITLE,news.getTitle());
            values.put(DbHelper.COLUMN_NEWS_URL,news.getUrl());
    ...
    public List<News> loadFavorite(){
        ...
        if (cursor.moveToLast()){//从最后往前加载，最新收藏的消息才排在最前面
            do {
                ...
                news.setImgUrl(cursor.getString(2));
